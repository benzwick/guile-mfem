;; SPDX-FileCopyrightText: 2026 Benjamin F. Zwick
;; SPDX-License-Identifier: BSD-3-Clause

(use-modules (srfi srfi-64))

;; MFEM data directory (set by CTest via MFEM_DATA_DIR environment variable)
(define mfem-data-dir (getenv "MFEM_DATA_DIR"))
(define guile-load-path (getenv "LTDL_LIBRARY_PATH"))
(define project-root (dirname (dirname (dirname (current-filename)))))
(define ex2-script (string-append project-root "/examples/ex2-lisp.scm"))

(test-begin "mfem-ex2-lisp")

(test-group "elasticity"
  (let* ((mesh-file (string-append mfem-data-dir "/beam-tri.mesh"))
         (rc (system* "guile"
                      "-L" guile-load-path
                      ex2-script
                      "--" "-m" mesh-file)))
    (test-assert "ex2-lisp exits successfully" (zero? (status:exit-val rc)))
    (test-assert "sol.gf exists" (file-exists? "sol.gf"))
    (test-assert "displaced.mesh exists" (file-exists? "displaced.mesh"))
    (test-assert "sol.gf non-empty"
      (> (stat:size (stat "sol.gf")) 100))))

(define runner (test-runner-current))
(test-end "mfem-ex2-lisp")
(exit (zero? (test-runner-fail-count runner)))
